        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <ul className="space-y-6">
            <li className="flex gap-4 items-start">
              <div className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-700 text-white flex items-center justify-center text-[10px] font-black mt-1">
                1
              </div>
              <p className="text-slate-800 leading-relaxed font-bold">
                今回の価格改定により、年間で{' '}
                <span className="font-black text-blue-700 text-xl border-b-2 border-blue-200 px-1">
                  {formatCurrency(results.annualNetImprovement)}円
                </span>{' '}
                のキャッシュフロー改善（実質利益増）が期待できます。これは月換算で約{' '}
                <span className="font-bold text-slate-900">
                  {formatCurrency(results.monthlyImprovement)}円
                </span>{' '}
                のインパクトです。
              </p>
            </li>
            <li className="flex gap-4 items-start">
              ...
              <p className="text-slate-800 leading-relaxed font-bold">
                増収分のうち{' '}
                <span className="font-black text-blue-700 text-xl border-b-2 border-blue-200 px-1">
                  {formatCurrency(results.promotionCost)}円
                </span>{' '}
                を戦略的に販促や顧客維持に再投資することで、値上げに伴う離脱リスクを最小限に抑える設計となっています。
              </p>
            </li>
          </ul>
          ...
          <div className="p-6 rounded-2xl bg-gradient-to-br from-blue-600 to-blue-800 text-white shadow-lg">
            {results.improvedRunway === 'no_short' ? (
              ...
              <div className="flex flex-col h-full justify-center">
                ...
                <p className="text-blue-50 leading-relaxed font-bold text-lg">
                  現在の予測では、資金ショートの時期を約{' '}
                  <span className="text-3xl font-black text-white px-1 underline decoration-blue-300 decoration-4 underline-offset-4">
